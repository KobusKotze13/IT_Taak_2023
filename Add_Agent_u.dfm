object frmAddAgent: TfrmAddAgent
  Left = 0
  Top = 0
  Caption = 'Add Agent'
  ClientHeight = 686
  ClientWidth = 1022
  Color = clBackground
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  OnClose = FormClose
  OnCreate = FormCreate
  OnShow = FormShow
  TextHeight = 15
  object gplPOS: TGridPanel
    Left = 0
    Top = 0
    Width = 1022
    Height = 686
    Align = alClient
    ColumnCollection = <
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end>
    ControlCollection = <
      item
        Column = 7
        ColumnSpan = 3
        Control = pnlMainMenubtn
        Row = 9
        RowSpan = 2
      end
      item
        Column = 1
        Control = lblEmailAddress
        Row = 6
      end
      item
        Column = 1
        Control = lblCellphoneNum
        Row = 5
      end
      item
        Column = 1
        Control = lblIDNum
        Row = 4
      end
      item
        Column = 1
        Control = lblSurname
        Row = 3
      end
      item
        Column = 3
        ColumnSpan = 3
        Control = edtSurname
        Row = 3
      end
      item
        Column = 1
        ColumnSpan = 3
        Row = 0
      end
      item
        Column = 3
        ColumnSpan = 3
        Control = edtCellphoneNum
        Row = 5
      end
      item
        Column = 3
        ColumnSpan = 3
        Control = edtEmail
        Row = 6
      end
      item
        Column = 1
        Control = lblPassword
        Row = 7
      end
      item
        Column = 3
        ColumnSpan = 3
        Control = edtPassword
        Row = 7
      end
      item
        Column = 1
        Control = lblName
        Row = 2
      end
      item
        Column = 3
        ColumnSpan = 3
        Control = edtName
        Row = 2
      end
      item
        Column = 1
        Control = lblConfirmPassword
        Row = 8
      end
      item
        Column = 3
        ColumnSpan = 3
        Control = edtConfirmPassword
        Row = 8
      end
      item
        Column = 3
        ColumnSpan = 3
        Control = edtIDNum
        Row = 4
      end>
    RowCollection = <
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        Value = 10.000000000000000000
      end
      item
        SizeStyle = ssAuto
      end>
    TabOrder = 0
    ExplicitHeight = 680
    DesignSize = (
      1022
      686)
    object pnlMainMenubtn: TPanel
      Left = 755
      Top = 621
      Width = 225
      Height = 59
      Anchors = []
      Caption = 'Back to Main Menu'
      TabOrder = 0
    end
    object lblEmailAddress: TLabel
      Left = 131
      Top = 433
      Width = 46
      Height = 25
      Anchors = [akLeft, akRight]
      Caption = 'Email'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clHighlight
      Font.Height = -18
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lblCellphoneNum: TLabel
      Left = 111
      Top = 364
      Width = 85
      Height = 25
      Anchors = [akLeft, akRight]
      Caption = 'Cellphone'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clHighlight
      Font.Height = -18
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lblIDNum: TLabel
      Left = 107
      Top = 296
      Width = 93
      Height = 25
      Anchors = [akLeft, akRight]
      Caption = 'ID Number'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clHighlight
      Font.Height = -18
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lblSurname: TLabel
      Left = 116
      Top = 228
      Width = 75
      Height = 25
      Anchors = [akLeft, akRight]
      Caption = 'Surname'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clHighlight
      Font.Height = -18
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold]
      ParentFont = False
      ExplicitTop = 224
    end
    object edtSurname: TEdit
      Left = 367
      Top = 229
      Width = 186
      Height = 23
      Anchors = [akLeft, akRight]
      TabOrder = 1
    end
    object edtCellphoneNum: TEdit
      Left = 367
      Top = 365
      Width = 186
      Height = 23
      Anchors = []
      TabOrder = 2
      ExplicitLeft = 163
    end
    object edtEmail: TEdit
      Left = 367
      Top = 434
      Width = 186
      Height = 23
      Anchors = []
      TabOrder = 3
      ExplicitLeft = 163
    end
    object lblPassword: TLabel
      Left = 114
      Top = 501
      Width = 80
      Height = 25
      Anchors = [akLeft, akRight]
      Caption = 'Password'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clHighlight
      Font.Height = -18
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object edtPassword: TEdit
      Left = 367
      Top = 502
      Width = 186
      Height = 23
      Anchors = []
      TabOrder = 4
      ExplicitLeft = 163
    end
    object lblName: TLabel
      Left = 129
      Top = 159
      Width = 50
      Height = 25
      Anchors = [akLeft, akRight]
      Caption = 'Name'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clHighlight
      Font.Height = -18
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object edtName: TEdit
      Left = 367
      Top = 160
      Width = 186
      Height = 23
      Anchors = [akLeft, akRight]
      TabOrder = 5
      ExplicitTop = 164
    end
    object lblConfirmPassword: TLabel
      Left = 114
      Top = 557
      Width = 80
      Height = 50
      Alignment = taCenter
      Anchors = [akLeft, akRight]
      Caption = 'Confirm'#13'Password'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clHighlight
      Font.Height = -18
      Font.Name = 'Segoe UI'
      Font.Style = [fsBold]
      ParentFont = False
      ExplicitTop = 568
    end
    object edtConfirmPassword: TEdit
      Left = 367
      Top = 571
      Width = 186
      Height = 23
      Anchors = []
      TabOrder = 6
      ExplicitLeft = 163
    end
    object edtIDNum: TEdit
      Left = 367
      Top = 297
      Width = 186
      Height = 23
      Anchors = [akLeft, akRight]
      TabOrder = 7
      ExplicitTop = 301
    end
  end
end
